...
const Blog: React.FC<Iprops> = ({ data, slug }) => {
    ...
    return (
        <Fragment>
            <Head>
                <title key={1}>{menu?.contactInfo?.[LINKS_FROM_MENU_TITLES.blogs].title} | Virginia Beach Auto Transport CRM Blog</title>,
                <meta key={2}
                    property="og:title"
                    data-hid="og:title"
                    data-n-head="ssr"
                    content={`Virginia Beach Auto Transport Website ${menu?.contactInfo?.[LINKS_FROM_MENU_TITLES.blogs].title}`}
                ></meta>
            </Head>
            <section className={classes.mt160}>
                <Container>
                    <div className={classes.blogSection}>
                        ...
                        <Conditional condition={dataLatestBlogs?.data}>
                            <LatestNews latestBlogs={dataLatestBlogs.data}/>
                        </Conditional>
                    </div>
                </Container>
            </section>
