...
const Blog: React.FC<Iprops> = ({ data, slug }) => {
    const { data: dataLatestBlogs } = useQuery<Record<string, any> | any>('latestNews', getLatestNewsNew(2));
    ...
    return (
        <Fragment>
            <Head>
                <title key={1}>{menu?.contactInfo?.[LINKS_FROM_MENU_TITLES.blogs].title} | SAN-JOSE Car Transport CRM Blog</title>,
                <meta key={2}
                    property="og:title"
                    data-hid="og:title"
                    data-n-head="ssr"
