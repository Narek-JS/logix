...
import { getLatestBlogsBlog, getLatestNewsNew } from '@/constants/service';
...
const CurrentNew: React.FC<Iprops> = ({ data, slug }) => {
    const { data: dataLatestNews } = useQuery<Record<string, any> | any>('latestNews', getLatestNewsNew(2));
    const { data: dataLatestBlogs } = useQuery<Record<string, any> | any>('latestBlogs', getLatestBlogsBlog(4));
    ...
    return (
        <Fragment>
            <Head>
                <title key={1}>{menu?.contactInfo?.[LINKS_FROM_MENU_TITLES.blogs].title} | SAN-JOSE Car Transport Website CRM New</title>,
                <meta key={2}
                    property="og:title"
                    data-hid="og:title"
                    data-n-head="ssr"
                    content={`SAN-JOSE Car Transport Website ${menu?.contactInfo?.[LINKS_FROM_MENU_TITLES.blogs].title}`}
                ></meta>
            </Head>
            ...
            <section>
                <Container>
                    <h3 className={classes.contentBlocksTitle}>You Might Also Like</h3>
                    <div className={classes.contentBlocks}>
                        { dataLatestBlogs?.data?.map((news, index) => ( 
                            <PostCard
                                categoryName='news'
